...
export function BaseNode({
  ...
  const isInspectHighlighted =
    simStatus === 'inspecting' &&
    inspectTrace &&
    inspectTrace[inspectIdx]?.nodeId === id
  ...
  return (
    <div
      className={cn(
        'relative min-w-[140px] rounded-xl border backdrop-blur-sm transition-all duration-200 cursor-pointer select-none',
    ...
    >
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[11px] font-semibold text-white/80 truncate max-w-[90px]">
            {label}
          </span>
          {/* Status dot */}
          <div
            className={cn(
              'w-2 h-2 rounded-full ml-auto flex-shrink-0 transition-colors',
              statusColors[status],
        ...
        {/* Metrics overlay (during simulation) */}
      ...
      {(hasTarget || showBothHandles) && (
        <Handle
          ...
          className="!w-2.5 !h-2.5 !border-2 !border-white/30 !bg-[#1a1a24] hover:!border-white/70 transition-colors"
        />
      )}
...
function MetricItem({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="flex gap-1 items-baseline">
      <span className="text-[9px] text-white/40 font-mono uppercase">{label}</span>
