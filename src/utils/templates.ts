...
function loadThreeTier() {
  ...
  updateNodeConfig(clientId, { nodeType: 'client', rps: 8, method: 'GET', path: '/api/feed' })
...
function loadLoadBalanced() {
  ...
  updateNodeConfig(clientId, { nodeType: 'client', rps: 10, method: 'GET', path: '/api/users' })
...
function loadMicroservices() {
  ...
  const clientId   = addNode('client',       { x: 60,  y: 280 })
  const gatewayId  = addNode('gateway',      { x: 280, y: 280 })
  const lbId       = addNode('load-balancer',{ x: 500, y: 280 })
  const authSvcId  = addNode('service',      { x: 740, y: 120 })
  const prodSvcId  = addNode('service',      { x: 740, y: 440 })
  const authDbId   = addNode('database',     { x: 980, y: 120 })
  const prodCacheId = addNode('cache',       { x: 980, y: 370 })
  const prodDbId   = addNode('database',     { x: 1220, y: 370 })

  updateNodeLabel(clientId,   'Mobile App')
  updateNodeLabel(gatewayId,  'API Gateway')
  updateNodeLabel(lbId,       'Router')
  ...
  onConnect({ source: clientId,    target: gatewayId,   sourceHandle: null, targetHandle: null })
  onConnect({ source: gatewayId,   target: lbId,        sourceHandle: null, targetHandle: null })
  onConnect({ source: lbId,        target: authSvcId,   sourceHandle: null, targetHandle: null })
  onConnect({ source: lbId,        target: prodSvcId,   sourceHandle: null, targetHandle: null })
  ...
  updateNodeConfig(gatewayId, {
    nodeType: 'gateway',
    rules: [
      { id: nanoid(6), method: 'GET', pathPrefix: '/api', targetNodeId: lbId },
      { id: nanoid(6), method: 'POST', pathPrefix: '/api', targetNodeId: lbId },
    ],
  })

  updateNodeConfig(clientId,  { nodeType: 'client', rps: 12, method: 'GET', path: '/api/products' })
